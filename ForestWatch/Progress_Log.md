...
* *Issue:* I couldn't create lsass.exe process dump rule
* *Solution:* Mimikatz is not working on default Win 2025 installation -> Used procdump -> Reconfigured sysmon to log Event 10 when TargetImage=lsass.exe -> Rule still not triggered(default Wazuh ruleset contained rule 61612 with level=0 <- was successfully muting sysmon alerts to prevent noise)
